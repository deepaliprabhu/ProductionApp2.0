...
- (void)syncProcesses:(NSMutableArray*)processesArray withProcessData:(NSMutableDictionary*)processData {
    ...
    if (count > 0) {
        jsonString = [NSString stringWithFormat:@"&process_ctrl_id=%@&process_ctrl_name=%@&alias=%@&originator=%@&approver=%@&comments=%@&timestamp=%@&description=%@&productid=%@&pcb_productid=%@&versionid=%@&status=%@&count=%lu&json=%@",[self urlEncodeUsingEncoding:processData[@"process_ctrl_id"]], [self urlEncodeUsingEncoding:processData[@"process_ctrl_name"]],[self urlEncodeUsingEncoding:processData[@"alias"]], processData[@"Originator"], processData[@"Approver"],[self urlEncodeUsingEncoding:processData[@"Comments"]], processData[@"Timestamp"], [self urlEncodeUsingEncoding:processData[@"Description"]],[self urlEncodeUsingEncoding:processData[@"ProductId"]], [self urlEncodeUsingEncoding:processData[@"PCBProductId"]],processData[@"VersionId"],[self urlEncodeUsingEncoding:processData[@"Status"]],(unsigned long)processesArray.count,jsonString];
        [__ServerManager setDelegate:self];
...
- (NSMutableArray*)reorderProcessSteps:(NSMutableArray*)processesArray {
    for (int i=0; i < processesArray.count-1; ++i) {
        for (int j=i+1; j < processesArray.count; ++j) {
            NSMutableDictionary *processDatai = processesArray[i];
            NSMutableDictionary *processDataj = processesArray[j];
            int processNoi = [processDatai[@"stepid"] intValue];
            int processNoj = [processDataj[@"stepid"] intValue];
            if (processNoi>processNoj) {
                NSMutableDictionary *tempDict = processDatai;
                processesArray[i] = processesArray[j];
                processesArray[j] = tempDict;
